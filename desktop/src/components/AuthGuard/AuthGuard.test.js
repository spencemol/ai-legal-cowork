...
describe('AuthGuard (Task 5.4)', () => {
    ...
    it('renders login fallback when not authenticated', async () => {
        ...
        render(_jsx(AuthGuard, { fallback: _jsx("div", { children: "Please login" }), children: _jsx("div", { children: "Protected content" }) }));
    ...
    it('transitions from login to protected when token is set', async () => {
        const { AuthGuard } = await import('./AuthGuard');
        const { rerender } = render(_jsx(AuthGuard, { fallback: _jsx("div", { children: "Please login" }), children: _jsx("div", { children: "Protected content" }) }));
        expect(screen.getByText(/please login/i)).toBeInTheDocument();
        useAuthStore.setState({
            token: 'valid-token',
            user: { id: '1', email: 'a@b.com', name: 'A', role: 'attorney' },
        });
        rerender(_jsx(AuthGuard, { fallback: _jsx("div", { children: "Please login" }), children: _jsx("div", { children: "Protected content" }) }));
        expect(screen.getByText(/protected content/i)).toBeInTheDocument();
    });
});
