...
describe('ChatMessage (Task 5.8)', () => {
    it('renders user message with correct role label', async () => {
        ...
        const message = { id: '1', role: 'user', content: 'What is the law?' };
        render(_jsx(ChatMessage, { message: message }));
    ...
    it('renders assistant message with correct role label', async () => {
        const { ChatMessage } = await import('./ChatMessage');
        const message = { id: '2', role: 'assistant', content: 'The law states...' };
        render(_jsx(ChatMessage, { message: message }));
    ...
    it('shows streaming indicator when isStreaming is true', async () => {
        const { ChatMessage } = await import('./ChatMessage');
        const message = { id: '4', role: 'assistant', content: 'Thinking...' };
        render(_jsx(ChatMessage, { message: message, isStreaming: true }));
    ...
    it('applies user message styling class', async () => {
        const { ChatMessage } = await import('./ChatMessage');
        const message = { id: '5', role: 'user', content: 'My question' };
        const { container } = render(_jsx(ChatMessage, { message: message }));
        expect(container.querySelector('.message-user')).toBeTruthy();
    });
    it('applies assistant message styling class', async () => {
        const { ChatMessage } = await import('./ChatMessage');
        const message = { id: '6', role: 'assistant', content: 'My answer' };
        const { container } = render(_jsx(ChatMessage, { message: message }));
