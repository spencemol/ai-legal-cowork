...
describe('CitationLink (Task 5.9)', () => {
    it('renders a citation number link', async () => {
        ...
        render(_jsx(CitationLink, { citation: mockCitation, index: 1, onCitationClick: vi.fn() }));
    ...
    it('calls onCitationClick with citation when clicked', async () => {
        const onClick = vi.fn();
        const { CitationLink } = await import('./CitationLink');
        render(_jsx(CitationLink, { citation: mockCitation, index: 1, onCitationClick: onClick }));
        fireEvent.click(screen.getByRole('button', { name: /\[1\]/i }));
        expect(onClick).toHaveBeenCalledWith(mockCitation);
    });
    it('displays page number in title when page is set', async () => {
        const { CitationLink } = await import('./CitationLink');
        render(_jsx(CitationLink, { citation: mockCitation, index: 2, onCitationClick: vi.fn() }));
    ...
    it('renders with citation-link class', async () => {
        const { CitationLink } = await import('./CitationLink');
        const { container } = render(_jsx(CitationLink, { citation: mockCitation, index: 1, onCitationClick: vi.fn() }));
        expect(container.querySelector('.citation-link')).toBeTruthy();
    });
    it('handles null page gracefully', async () => {
        const citationNoPage = { ...mockCitation, page: null };
        const { CitationLink } = await import('./CitationLink');
        render(_jsx(CitationLink, { citation: citationNoPage, index: 3, onCitationClick: vi.fn() }));
