...
describe('DocumentViewer (Tasks 5.12, 5.13)', () => {
    ...
    it('renders nothing when no citation is provided', async () => {
        ...
        const { container } = render(_jsx(DocumentViewer, { citation: null, onClose: vi.fn() }));
    ...
    it('renders document viewer panel when citation is provided', async () => {
        ...
        const { DocumentViewer } = await import('./DocumentViewer');
        render(_jsx(DocumentViewer, { citation: mockCitation, onClose: vi.fn() }));
        await waitFor(() => {
    ...
    it('calls onClose when close button is clicked', async () => {
        ...
        const onClose = vi.fn();
        const { DocumentViewer } = await import('./DocumentViewer');
        render(_jsx(DocumentViewer, { citation: mockCitation, onClose: onClose }));
        await waitFor(() => {
            expect(screen.getByRole('button', { name: /close/i })).toBeInTheDocument();
        });
